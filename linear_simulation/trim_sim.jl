...
#------------------------------------------------------------------------
s = ArgParseSettings()
s.description = "Trimmed Lasso Simulations"
...
##### simulations for Trimmed ℓ₁ penalty #####
println("\n##### Trimmed Lasso Simulation for (n, p, k) = (", n, ", ", p, ", ", k, ") with ", o[:type], " matrix #####")
...
for sim = 1:max_sims
    ...
    for λ in (10 .^ (collect(-3:0.2:1)))*n
        ...
        solve_trim_lasso(X, y, θ, w, h, λ; itm=itm, tol=1e-6, ptf=1000)
