...
#------------------------------------------------------------------------
s = ArgParseSettings()
s.description = "MCP Lasso Simulations"
...
##### simulations for MCP penalty #####
println("\n##### MCP Lasso Simulation for (n, p, k) = (", n, ", ", p, ", ", k, ") with ", o[:type], " matrix #####")
...
for sim = 1:max_sims
    ...
    for λ in (10 .^ (collect(-3:0.2:1)))*n
        ...
        solve_mcp_lasso(X, y, θ, b, λ; itm=itm, tol=1e-6, ptf=1000)
